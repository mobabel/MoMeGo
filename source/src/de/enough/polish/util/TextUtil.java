...
public final class TextUtil {
	...
	public static String[] wrap( String value, Font font, int firstLineWidth, int lineWidth ) {
		...
		//#if {cfg.osHasFontAlready} != true
			int completeWidth = UnicodeFunc.stringUtf8Width(value, font);
		//#else
		...
		if (!hasLineBreaks) {
			...
			for (int i = 0; i < valueChars.length; i++) {
				...
				if (c == '\n' || i == valueChars.length -1 || isCRLF ) {
					...
					//lee
					//#if {cfg.osHasFontAlready} != true
						completeWidth = UnicodeFunc.stringUtf8Width(line, font);
					//#else
			...
			ArrayList list ) 
	{
		...
		for (int i = 0; i < valueChars.length; i++) {
			char c = valueChars[i];
			//#if {cfg.osHasFontAlready} != true
			currentLineWidth += UnicodeFunc.stringUtf8Width(c+"", font);
			//#else
