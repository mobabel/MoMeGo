...
//#endif
{
	...
	//#if tmp.directInput
		...
		//#if {cfg.useLocalInput} == true
			public static final int MODE_PINYIN = 4;
			/** input mode for input using a separete native TextBox */
			//lee 4->5
			public static final int MODE_NATIVE = 5;
		//#else
			/** input mode for input using a separete native TextBox */
		...
		//#else
		private static final String charactersKey1 = ".,!?\u00bf:/()@_-+1'\";[]*";//lee add ;[]*
		//#endif
		...
		//#else
			protected static final String charactersKeyStar = ".,!?\u00bf:/@_-+1'\";[]*";//lee add ;[]*
		//#endif
	...
	//lee
	//#if {cfg.useLocalInput} == true
		private InputMethodUtf8 imt = null;
	//#endif
	...
	public TextField( String label, String text, int maxSize, int constraints, Style style)
	{
		...
		//#if {cfg.useLocalInput} == true
			...
			imt = InputMethodUtf8.getInstance(getScreen());
			//#else
			imt = InputMethodUtf8.getInstance();
			//#endif
			imt.clearCache();
			imt.clsContent();
			imt.keyAvailable.removeAllElements();
			imt.stateUtfInputNow = false;
		
			//#if tmp.directInput
				imt.inputMode = MODE_LOWERCASE;
			//#endif
		//#endif
...
/*			if (style != null && this.helpItem.style != null && this.helpItem.style.font == null) {
	...
	{
		//#debug warn
		System.out.println("call getCaretPosition");
	...
	public void setCaretPosition(int position) {
		// #debug warn
		//System.out.println("call setCaretPosition");
		//#if polish.blackberry
...
//		}
	...
	public void paintContent(int x, int y, int leftBorder, int rightBorder, Graphics g) {
		//lee, set the Pinyin box position
		//#if {cfg.useLocalInput} == true
			imt.setPinYinBoxStartY(y, leftBorder);
		//#endif
		...
		//#ifdef tmp.directInput
			...
			//#endif
			    ...
			    //lee
			    //#if {cfg.useLocalInput} == true	
		        	if( (inputMode != TextField.MODE_PINYIN) || (inputMode == TextField.MODE_PINYIN && !imt.stateUtfInputNow)){
			    //#endif	
					if (this.showCaret) {
						//#ifdef polish.css.textfield-caret-color
		        ...
		        //#if {cfg.useLocalInput} == true	
						...
						//lee, do not need to draw the caretChar(abc.. pressed letter) 
			        	//#if {cfg.useLocalInput} == true
						this.caretChar = UnicodeFunc.charNull;
						//#endif
						// #debug warn
						//System.out.println("this.text: " + this.text);
			        }
			        //#endif
					//lee, paint the input dialog
			    	//#if {cfg.useLocalInput} == true
		        		if (inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
		        			// #debug warn
		        			//System.out.println("call the paintInput");
			        		imt.paintInput(g, this.font);
		        		}
			        //#endif
	...
	protected void initContent(int firstLineWidth, int lineWidth) {
		// #debug warn
		//System.out.println("call initContent" );
		this.screen = getScreen();
		//#if tmp.includeInputInfo
...
//				}
	...
	private void setCaretRow( String line, int column ) {
		...
		//lee
		//#if {cfg.useLocalInput} == true
			caretRowFirstPart = UnicodeFunc.StringDeleteCharNull(caretRowFirstPart);
		//#endif
...
//	}
	...
	protected void commitCurrentCharacter() {
		// #debug warn
		//System.out.println("commitCurrentCharacter: " + this.caretChar+" " +text);
		String myText;
		if (this.isPassword) {
		...
		//lee, add condition, ontherwise caret will keep moving right when input pinyin
		//#if {cfg.useLocalInput} == true
		if( (inputMode != TextField.MODE_PINYIN) || (inputMode == TextField.MODE_PINYIN && !imt.stateUtfInputNow))
		//#endif
		if (this.isPassword) {
	...
	protected void insertCharacter( char insertChar, boolean append, boolean commit ) {	
        ...
        //#if {cfg.useLocalInput} == true	
			// #debug warn
			//System.out.println( "insertCharacter- stateUtfInputNow: "+ text +" " + imt.stateUtfInputNow );
		//#endif
		//lee
		//#if {cfg.useLocalInput} == true
			this.text = UnicodeFunc.StringDeleteCharNull(this.text);
		//#endif
		...
		if (myText == null || myText.length() == 0) {
			...
			//#if {cfg.useLocalInput} == true
			if( inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow)
				if(imt.content[0].length()>0 && this.text != null)
					myText = ""+imt.content[0];
				else
					myText = "";
			else
			//#endif	
			...
			//TODO bug lee, if caretpos >text length, it is wrong
			/*if(cp>myText.length()){
				cp=myText.length()<1?0:myText.length()-1;
			}*/
			//lee
			//#if {cfg.useLocalInput} == true
			if( inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow)
				if(imt.content[0].length()>0 && this.text != null){
					buffer.append( myText.substring( 0, cp ) ).append(imt.content[0]);
					// #debug warn
					//System.out.println("The last content in text is: "+ ItemUTF8Lib.getInstance().getTableUtf8LocalCode().get(imt.content[0]).toString());
				}else{
		...
		//#if {cfg.useLocalInput} == true
			if(myText.length() > 0 )
				myText = UnicodeFunc.StringDeleteCharNull(myText);
			//lee avoid that the first pinyin has no delete function
			else if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow && myText.length()==0)
				myText = String.valueOf(UnicodeFunc.charNull);
		//#endif
		//lee add condition
		//#if {cfg.useLocalInput} == true	
			if(!myText.equals(""))
		...
		if (!commit) {
			if (myText.length() == 1) {
				//lee  fix bug when has input 1 char already, if go on input, the caret will be 0,
				// and the new char will be the first place(wrong)
				//#if {cfg.useLocalInput} == true
	 ...
	 * @return
	 */
	public boolean handleKeyInput(int keyCode, int gameAction){
		// #debug warn
		//System.out.println("handleKeyInput: "+keyCode+" "+gameAction);
        //#if {cfg.useLocalInput} == true
		try{
			//#if polish.key.RightSoftKey:defined
			//#= if (keyCode == ${polish.key.RightSoftKey} )
			//#endif
			{	
				// #debug warn
				//System.out.println("handleKeyInput RightSoftKey: ");
				/*if(imt.keyAvailable==null){
					imt.clsContent();
				}else if(imt.keyAvailable.size()==0){
					imt.clsContent();
				}
				else{//-7 is backspace or clear key?
					imt.keyPressed(-7, this.inputMode);
				}*/
			}
			//does not work now, back button replace it
			//#if polish.key.LeftSoftKey:defined
			//#= else if (keyCode == ${polish.key.LeftSoftKey} )
			//#endif
			{
				// #debug warn
				//System.out.println("handleKeyInput LeftSoftKey: ");
				/*if(imt.keyAvailable!=null){
					if(imt.keyAvailable.size()==0){
						if(imt.content[0]!=null && imt.content[0].length()>0){
							...
							// #debug warn
							//System.out.println("Sorry, the content is empty");
						}
					}
				}*/
			}
			//#if polish.key.RightSoftKey:defined or polish.key.LeftSoftKey:defined
			// #= else
			//#endif
			{
				imt.keyPressed(keyCode, this.inputMode);
...
/*			if(imt.content[0].length()>0 && this.text != null){
				this.text = UnicodeFunc.StringDeleteCharNull(this.text);
				this.text += imt.content[0];
				this.setString(this.text);
				//#if {cfg.osHasFontAlready} != true
					//#debug warn
					System.out.println("The last content in text is: "+ ItemUTF8Lib.getInstance().getTableUtf8LocalCode().get(imt.content[0]).toString());
				//#endif
				//do not store string in imt, but in text
				imt.clsContent();
			}
			imt.refreshContent();
			*/
			...
			//#debug error
			System.out.println("Error in handleKeyInput: "+e.getMessage());
			return false;
		}
		//#endif
		return true;
		...
		//#endif
				//#ifdef tmp.directInput
					//#if !polish.blackberry
						...
						//lee
						//#if {cfg.useLocalInput} == true
						if (this.inputMode == MODE_PINYIN && keyCode != KEY_CHANGE_MODE) {
							//call this anyway
							handleKeyInput(keyCode, gameAction);
						}
						//#endif
					//#endif
	...
	{		
		// #debug warn
		//System.out.println("call handleKeyInsert");
		//#if tmp.directInput
			//#if tmp.usePredictiveInput
				if (this.predictiveInput) {
					//#if tmp.supportsAsciiKeyMap
			...
			{	
				...
				//lee, if utf, newCharacter is null tem char, not the element in "mno6ö"(for example) of button 6
				//#if {cfg.useLocalInput} == true
					if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
						newCharacter = UnicodeFunc.charNull;
					}
				//#endif
				// #debug warn
				...
				//#endif
						...
						//lee
						//#if {cfg.useLocalInput} == true		
							this.caretWidth = UnicodeFunc.stringUtf8Width(String.valueOf(newCharacter), this.font);
						//#endif
				//#ifdef polish.css.font-bitmap
					}
				//#endif
				...
				if (alphabetLength == 1) {
					...
					// #debug warn
					//System.out.println( "this is right test" );
					//lee add condition 
					//if( (inputMode != TextField.MODE_PINYIN) || (inputMode == TextField.MODE_PINYIN && !imt.stateUtfInputNow))
		...
		//#if tmp.directInput
			...
			//lee
				//#if {cfg.useLocalInput} == true
					if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
						//#debug warn
						//System.out.println("handleKeyClear-Clear input");
						if(imt.keyAvailable==null){
							imt.clsContent();
						}else if(imt.keyAvailable.size()==0){
							imt.clsContent();
							...
							//#endif
							//dont use this
							//imt.keyPressed(InputMethodUtf8.KEY_BACKSPACE, this.inputMode);
						}
						imt.refreshContent();
						return false;
					}else
				//#endif
					{
						// #debug warn
						//System.out.println("handleKeyClear-Clear char " + this.text.length());
						if ( this.text != null && this.text.length() > 0) {			
	...
	{
		// #debug warn
		//System.out.println("handleKeyMode");
	...
	{
		// #debug warn
		//System.out.println("handleKeyNavigation");
		//#if tmp.directInput
			...
			//#if {cfg.useLocalInput} == true
				if( (inputMode != TextField.MODE_PINYIN) || (inputMode == TextField.MODE_PINYIN && !imt.stateUtfInputNow))
					characterInserted = (character != this.editingCaretChar && character != UnicodeFunc.charNull);
			//#endif
			...
			//#if {cfg.useLocalInput} == true
				...
				// #debug warn
				//System.out.println("just see Fire!!");
				if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
					imt.doFire();
					handleKeyInput(keyCode, gameAction);
					insertCharacter(UnicodeFunc.charNull, true, true);
					imt.clsContent();
					imt.refreshContent();
			...
			//#endif
				...
				// #debug warn
				//System.out.println("just see up!!");
				//lee
				//#if {cfg.useLocalInput} == true
				if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
					imt.doUp();
		        ...
		        //#if {cfg.useLocalInput} == true	
				// #debug warn
				//System.out.println("just see down!! " +imt.stateUtfInputNow);
				//#endif
				//#if {cfg.useLocalInput} == true
				if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
					imt.doDown();	
			...
			//#endif
				//lee
				// #debug warn
				//System.out.println("just see left!!" + imt.stateUtfInputNow);
				//#if {cfg.useLocalInput} == true
				if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
					imt.doLeft();
					return true;	
			...
			//#endif
		        //#if {cfg.useLocalInput} == true	
				// #debug warn
				//System.out.println("just see right!!" + imt.stateUtfInputNow);
				//#endif
				//lee
				//#if {cfg.useLocalInput} == true
				if(inputMode == TextField.MODE_PINYIN && imt.stateUtfInputNow){
